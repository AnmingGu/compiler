(* ****** ****** *)
// DONE (lazy0)
fun ints_from(n0: int): lazy(strm(int)) =
  $lazy(strm_cons(n0, ints_from(n0+1)))

(* ****** ****** *)

fun sieve(xs: lazy(strm(int))): lazy(strm(int)) = 
$lazy(
  let
    val xs = $eval(xs)
    ...
    where {
      ...
      $lazy(
        let
          ...
          val xs = strm_uncons2(xs)
        in
          if (x1%x0 = 0) then $eval(filter(xs))
